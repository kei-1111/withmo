...
class UserSettingsRepositoryImplTest {
    ...
    fun `デフォルト設定を取得できること`() = runTest(testDispatcher) {
        ...
        repository.userSettings.test {
            ...
            assertEquals(false, data.notificationSettings.isNotificationAnimationEnabled)
            assertEquals(false, data.notificationSettings.isNotificationBadgeEnabled)
            assertEquals(true, data.clockSettings.isClockShown)
            assertEquals(ClockType.TOP_DATE, data.clockSettings.clockType)
            assertEquals(AppIconShape.Circle, data.appIconSettings.appIconShape)
            assertEquals(20f, data.appIconSettings.roundedCornerPercent)
            assertEquals(SortType.ALPHABETICAL, data.sortSettings.sortType)
            assertEquals(true, data.sideButtonSettings.isShowScaleSliderButtonShown)
            assertEquals(ThemeType.TIME_BASED, data.themeSettings.themeType)
            assertEquals(null, data.modelFilePath.path)
            assertEquals(1.0f, data.modelSettings.scale)
